...
readonly -a TEST_ASSET_NAMES=(
    SHA256SUMS
    serial-platform-v0.8.0-linux-x86_64-ubuntu20.04.tar.gz
    serial-platform-v0.8.0-macos-aarch64.zip
    serial-platform-v0.8.0-macos-x86_64.zip
    serial-platform-v0.8.0-windows-x86_64.zip
)
...
expect_failure "draft changed before publish" assert_release_snapshot \
    "${published_release}" 4242 v0.8.0 true
assert_verified_release_snapshot \
    "${published_release}" 4242 v0.8.0 false \
...
expect_failure "published snapshot missing asset" assert_verified_release_snapshot \
    "${published_missing_asset}" 4242 v0.8.0 false \
...
discover_release_by_tag v0.8.0 "${discovered_release}"
...
assert_release_snapshot "${id_snapshot}" 4242 v0.8.0 true
...
MOCK_GH_MODE=zero
if discover_release_by_tag v0.8.0 "${TEST_TEMP_DIR}/zero-release.json"; then
...
assert_verified_release_snapshot \
    "${post_publish_snapshot}" 4242 v0.8.0 false \
...
expect_failure "post-publish state remained draft" assert_verified_release_snapshot \
    "${valid_release}" 4242 v0.8.0 false \
...
expect_failure "post-publish asset set changed" assert_verified_release_snapshot \
    "${TEST_TEMP_DIR}/bad-post-assets.json" 4242 v0.8.0 false \

...
Serial Platform Desktop 是 Electron + React 桌面客户端。它与 TUI 使用相同的 `seriald` HTTP/WebSocket protocol v7，不直接打开物理串口。
...
- `src/main`：本地服务生命周期、protocol v7 HTTP/WebSocket client、snapshot/timeline 协调和 IPC handler；

...
- 当前串口配置中，按 `→` 展开端口/Profile/离散参数选项，`↑` / `↓` 选择，Enter 应用并折叠，`←` 折叠或返回；“机型 Profile”只选交互行为，独立的“机型名”按“一级机型系列 → 二级具体机型”选择。
- Shell/U-Boot 提示符和分段发送数值直接在当前行下输入；保存操作位于所有配置字段下方的独立操作区。
- “创建配置”包含“创建串口 Profile”“创建机型 Profile”和“配置机型名”。“配置机型名”先选“新增一级机型名”或已有系列，再进入系列新增/查看二级具体机型；新名称在当前行下直接输入，`←` 逐级返回。
